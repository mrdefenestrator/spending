...
# Four transactions in April 2026 (current month per project convention).
...
def _start_server(port: int, db_path: Path) -> subprocess.Popen:
    ...
    env = {
        **os.environ,
        ...
        # Port 1 is never open in tests → instant ECONNREFUSED
...
# ---------------------------------------------------------------------------
# Session-scoped server (empty DB) — used by test_navigation.py
# ---------------------------------------------------------------------------
...
def ofx_file(tmp_path):
    """OFX file with 4 transactions in 04/2026 (matches _SEEDED_OFX)."""
